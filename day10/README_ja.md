...
- **スタックの仕組み**: 後入れ先出し (LIFO) 構造の理解。
  - **例え**: ビュッフェにある「お皿の山」を想像してください。一番上に置くこと（Push）と、一番上から取ること（Pop/Pull）しかできません。または、ブラウザの **「戻る」ボタンの履歴** も同じです（最後に見たページが最初に戻るページ）。
...
- **テストプログラム**:
    ...
    SBC #$01   ; A = 3, C = 1
    ```
- **シミュレーション**: `make sim` を実行し、SBC の結果とキャリー（ボロー）フラグが期待通りに動作し、最終的に `PASS` と表示されることを確認します。
