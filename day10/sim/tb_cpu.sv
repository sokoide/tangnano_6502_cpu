// Day 10: Stack Operations - Logic Testbench
`timescale 1ns / 1ps
...
module tb_cpu;
    ...
    initial begin
        ...
        // 1. Initial Load A=0x42, S=0xFF
        repeat (2) @(posedge clk);
        #5;
        if (debug_a !== 8'h42 || debug_s !== 8'hFF) begin
            $display("FAIL: Initial state wrong, A=%h, S=%h", debug_a, debug_s);
            error_count++;
        end

        // 2. PHA execution
        repeat (3) @(posedge clk);
        #5;  // Assuming PHA takes 3 cycles
        if (debug_s !== 8'hFE || mem[16'h01FF] !== 8'h42) begin
            $display("FAIL: PHA failed, S=%h, stack[0x1FF]=%h", debug_s, mem[16'h01FF]);
            error_count++;
        end else $display("PASS: PHA correctly pushed 0x42 to stack");
        ...
        #5;  // Assuming PLA takes 4 cycles
        if (debug_a !== 8'h42 || debug_s !== 8'hFF) begin
            $display("FAIL: PLA failed, A=%h, S=%h", debug_a, debug_s);
            error_count++;
        end else $display("PASS: PLA correctly pulled 0x42 from stack");
