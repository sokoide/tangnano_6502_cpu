...
module tb_cpu_registers;
    ...
    initial begin
        ...
        // Check reset values
        assert (reg_a == 8'h00)
        else $error("Reset: A should be 0x00");
        assert (reg_x == 8'h00)
        else $error("Reset: X should be 0x00");
        assert (reg_y == 8'h00)
        else $error("Reset: Y should be 0x00");
        assert (reg_sp == 8'hFF)
        else $error("Reset: SP should be 0xFF");
        assert (reg_pc == 16'h0200)
        else $error("Reset: PC should be 0x0200");
        assert (reg_p == 8'h20)
        else $error("Reset: P should be 0x20");
        ...
        #10;
        assert (reg_a == 8'h55)
        else $error("A register write failed");
        ...
        #10;
        assert (reg_x == 8'hAA)
        else $error("X register write failed");
        ...
        #10;
        assert (reg_y == 8'h33)
        else $error("Y register write failed");
        ...
        #10;
        assert (reg_sp == 8'hF0)
        else $error("SP register write failed");
        ...
        pc_write = 1'b0;
        #10;
        assert (reg_pc == 16'h1234)
        else $error("PC register write failed");
        ...
        #10;
        assert (reg_p == 8'hC3)
        else $error("P register write failed");
        ...
        #10;
        assert (reg_a == 8'h77)
        else $error("Simultaneous write A failed");
        assert (reg_x == 8'h77)
        else $error("Simultaneous write X failed");
        assert (reg_y == 8'h77)
        else $error("Simultaneous write Y failed");
        assert (reg_sp == 8'h77)
        else $error("Simultaneous write SP failed");
        assert (reg_pc == 16'h5678)
        else $error("Simultaneous write PC failed");
        assert (reg_p == 8'h77)
        else $error("Simultaneous write P failed");
