...
// Read the opcode and determine which category the instruction belongs to.
...
module simple_decoder (
    ...
    // -------------------------------------------------------------------------
    // TODO: Determine the instruction category based on the opcode.
    // TODO: オペコードに基づき、命令のカテゴリを判定してください。
    // -------------------------------------------------------------------------
    // Use a case statement to set the appropriate output flag (is_load, etc.) to 1 for each opcode.
    // case文を使用して、各オペコードに対して適切な出力フラグ (is_load等) を 1 に設定します。
    //
    // Examples of major instructions: / 主な命令の例:
    // - Load (LDA, LDX, LDY): 0xA9, 0xA2, 0xA0, etc. / 等
    // - Store (STA, STX, STY): 0x85, 0x86, 0x84, etc. / 等
    // - Arithmetic (ADC, SBC): 0x69, 0xE9, etc. / 等
    // - Branch (BPL, BMI, BNE, BEQ): 0x10, 0x30, 0xD0, 0xF0, etc. / 等
    //
    ...
    always_comb begin
        ...
        case (opcode)
            // Implementation example: LDA (Immediate) / 実装例: LDA (Immediate)
            8'hA9: is_load = 1'b1;
