            ...
            case (state)
                    ...
                    if (data_in == OP_HLT) begin
                        ...
                        address_bus <= pc + 1;
                                                                        case (data_in)
                                                                            ...
                                                                            end                                                        OP_JSR, OP_JMP_ABS, OP_LDA_ABS, OP_STA_ABS,
                            ...
                            OP_DEX: begin x <= x - 1; z <= ((x - 8'h01) == 8'h00); n <= (x - 8'h01) >> 7; pc <= pc + 1; address_bus <= pc + 1; state <= STATE_FETCH_OPCODE; end
                            OP_DEY: begin y <= y - 1; z <= ((y - 8'h01) == 8'h00); n <= (y - 8'h01) >> 7; pc <= pc + 1; address_bus <= pc + 1; state <= STATE_FETCH_OPCODE; end
                            ...
                            OP_ASL_A: begin c <= a[7]; a <= {a[6:0], 1'b0}; z <= ({a[6:0], 1'b0} == 8'h00); n <= a[6]; pc <= pc + 1; address_bus <= pc + 1; state <= STATE_FETCH_OPCODE; end
                            OP_LSR_A: begin c <= a[0]; a <= {1'b0, a[7:1]}; z <= ({1'b0, a[7:1]} == 8'h00); n <= 1'b0; pc <= pc + 1; address_bus <= pc + 1; state <= STATE_FETCH_OPCODE; end
                            OP_ROL_A: begin automatic logic old_c; old_c = c; c <= a[7]; a <= {a[6:0], old_c}; z <= ({a[6:0], old_c} == 8'h00); n <= a[6]; pc <= pc + 1; address_bus <= pc + 1; state <= STATE_FETCH_OPCODE; end
                            OP_ROR_A: begin automatic logic old_c; old_c = c; c <= a[0]; a <= {old_c, a[7:1]}; z <= ({old_c, a[7:1]} == 8'h00); n <= old_c; pc <= pc + 1; address_bus <= pc + 1; state <= STATE_FETCH_OPCODE; end
                ...
                STATE_FETCH_OPERAND: begin
                    case (current_opcode)
                        ...
                        OP_EOR_IMM: begin logic [7:0] res; res = a ^ data_in; a <= res; z <= (res == 8'h00); n <= res[7]; end
                        OP_SBC_IMM: begin logic [8:0] diff; diff = {1'b0, a} - {1'b0, data_in} - (c ? 9'h0 : 9'h1); a <= diff[7:0]; c <= !diff[8]; z <= (diff[7:0] == 8'h00); n <= diff[7]; v <= (a[7] != data_in[7]) && (a[7] != diff[7]); end
                        OP_CMP_IMM: begin logic [8:0] diff; diff = {1'b0, a} - {1'b0, data_in}; c <= !diff[8]; z <= (diff[7:0] == 8'h00); n <= diff[7]; end
                        OP_CPX_IMM: begin logic [8:0] diff; diff = {1'b0, x} - {1'b0, data_in}; c <= !diff[8]; z <= (diff[7:0] == 8'h00); n <= diff[7]; end
                        OP_CPY_IMM: begin logic [8:0] diff; diff = {1'b0, y} - {1'b0, data_in}; c <= !diff[8]; z <= (diff[7:0] == 8'h00); n <= diff[7]; end
                ...
                STATE_EXECUTE: begin
                    case (current_opcode)
                        OP_LDA_ZP, OP_LDA_ABS, OP_LDA_ABX, OP_LDA_ABY: begin
                            a <= data_in;
                            z <= (data_in == 8'h00);
                            n <= data_in[7];
                        end
