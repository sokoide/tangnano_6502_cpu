...
1. **`top_core.sv`**:
    ...
    - `cpu_registers` と `simple_decoder` をインスタンス化し、デモ用の信号線と接続します。
...
このように、このプロジェクトでは「高速に動く本番の回路」と「動作を目視で確認するための低速なデモ用（計測用）回路」を共存させる手法をとっています。`demo_` は「テスト」ではなく、**教育用の可視化・デモのための回路**であることを示す接頭語です。
