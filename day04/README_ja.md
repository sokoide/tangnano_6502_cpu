...
**課題:** もし CPU が VRAM に書き込もうとした全く同じ瞬間に、LCD コントローラが読み出そうとしたらどうなるでしょうか？通常のシングルポート RAM では衝突が起きてしまいます。

**解決策: デュアルポートメモリ (SDPB)**
私たちが使用する "SDPB" メモリブロックには、独立した 2 組のアドレス線とデータ線があります。

- **Port A (書き込み専用):** CPU クロックに接続。
- **Port B (読み出し専用):** LCD クロックに接続。
BSRAM ブロック内部のハードウェアが調停を行うため、複雑な同期ロジックを書くことなく、「いつでも書き込み」「いつでも読み出し」が可能になります。これはソフトウェアの共有メモリシステムに対する FPGA の大きな利点です！
