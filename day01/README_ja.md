...
ソフトウェアエンジニアにとって馴染み深い `int` は通常 32 ビットや 64 ビットですが、ハードウェア（HDL）の世界では **ビット幅（信号線の本数）を 1 ビット単位で明示的に指定** することが非常に重要です。

書き方の形式: `[ビット幅]'[基数][値]`

- **ビット幅**: その数値が何ビット（線の本数）を占めるかを指定します。
- **基数**: `d` (10 進数 / decimal), `h` (16 進数 / hex), `b` (2 進数 / binary) など。
- **値**: その進数での実際の値。

**例:**

- `1'b1`: **1ビット**の 2 進数で、値は 1（スイッチの ON/OFF や 1 本の信号線に相当）。
- `8'hFF`: **8ビット**の 16 進数で、値は 255（1 バイトのデータ）。
- `16'd10`: **16ビット**の 10 進数で、値は 10（6502 のアドレスバス幅に相当）。

**なぜ明示的に書く必要があるのか？**

1. **リソースの節約**: 1 ビットで済むフラグに 32 ビットの領域を割り当てると、FPGA 内部の貴重な物理リソース（LUT や配線）を無駄に消費します。
2. **警告の防止**: 単に `1` と書くと、多くのツールはデフォルトで「32 ビットの整数」として扱います。これを 8 ビットのレジスタに足そうとすると、ツールは「32 ビットを 8 ビットに切り捨てました」という警告（Truncation Warning）を出します。

このプロジェクトでは、警告のないクリーンなビルドを目指すため、`counter + 1'b1` のようにビット幅を意識した記述を徹底します。
