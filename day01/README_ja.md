...
- **基数**: `d` (10進数 / decimal), `h` (16進数 / hex), `b` (2進数 / binary) など。
- **値**: その進数での実際の値。

**例:**

- `1'b1`: **1ビット**の2進数で、値は 1（スイッチの ON/OFF や 1 本の信号線に相当）。
- `8'hFF`: **8ビット**の16進数で、値は 255（1 バイトのデータ）。
- `16'd10`: **16ビット**の10進数で、値は 10（6502 のアドレスバス幅に相当）。

**なぜ明示的に書く必要があるのか？**

1. **リソースの節約**: 1ビットで済むフラグに 32 ビットの領域を割り当てると、FPGA 内部の貴重な物理リソース（LUT や配線）を無駄に消費します。
2. **警告の防止**: 単に `1` と書くと、多くのツールはデフォルトで「32ビットの整数」として扱います。これを 8 ビットのレジスタに足そうとすると、ツールは「32ビットを8ビットに切り捨てました」という警告（Truncation Warning）を出します。
