...
-   CPU は PC が指す住所（アドレス）をメモリに伝えます。
-   すると、メモリからその住所にあるデータ（命令）が返ってきます。
-   通常、命令を 1 つ実行するたびに PC は次に進みます（インクリメント）。

Day 05 ではまだメモリを読みませんが、「PC が進んでいく」という基本動作だけを作り、その動きを目で見て確認します。
