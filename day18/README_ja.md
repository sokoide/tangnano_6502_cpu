...
Day 05 以降、**テストベンチ (`day18/sim/`) は完成した状態で提供されています。** 自分で作成したコードの正しさを検証するために活用してください。

- **テストプログラム**: これまで学んだすべての機能を駆使したプログラムを実行します。
- **シミュレーション**: `make sim` を実行し、システム全体が調和して動作し、最終的に `PASS` と表示されることを確認します。
