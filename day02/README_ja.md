...
**データ型:**
```systemverilog
wire [7:0] data_bus;   // 8bit ワイヤ、接続用のネット
...
-   `wire`: 物理的な配線を表します。値を保持できず、`assign` 文などで継続的に駆動される必要があります。モジュールの入力や出力に使われるのをよく見かけますが、これは一般的な慣習です。

`reg` 型は古いVerilogのキーワードです。`logic` はそのより柔軟な代替品と考えることができます。
