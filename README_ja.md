...
3. `make test` を実行してロジックを確認する (シミュレーション)。**これは「ユニットテスト」の実行にあたります。**
4. `make download` を実行して FPGA に書き込む (実機動作)。**これは「本番環境へのデプロイ」にあたります。**
...
- **ソフトウェア**:
  - **GOWIN EDA**: FPGA 向けのコードを合成するための IDE・ツールチェーン（ハードウェア版の GCC/Clang のようなもの）。
  - **Verilator**: PC 上で「ユニットテスト」を実行するための高速な SystemVerilog シミュレータ。
