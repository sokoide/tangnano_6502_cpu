...
分岐命令は「絶対アドレス」を指定せず、現在地からの「相対距離」を指定します。

**例え:**

- **絶対アドレッシング** は GPS 座標のようなものです: 「緯度 35.6895、経度 139.6917 へ行け。」
- **相対アドレッシング** は道案内のようなものです: 「ここから 3 歩進め」「5 歩戻れ。」

これにより、プログラムをメモリ上のどこに配置しても（再配置可能）正しく動作するという利点があります。
...
- **テストプログラム**:
    ...
    ADC #$01   ; A = 5, C = 0
    ```
- **シミュレーション**: `make sim` を実行し、演算結果とフラグが期待通りに変化し、最終的に `PASS` と表示されることを確認します。
- **実機 (FPGA)**: LCD で演算結果とフラグ (NVZC) が正しく変化することを確認します。
