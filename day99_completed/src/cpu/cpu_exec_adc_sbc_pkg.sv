...
package cpu_exec_adc_sbc_pkg;
  ...
  task automatic exec_adc_sbc(
      ...
      ref logic flg_c, ref logic flg_v, ref logic flg_z, ref logic flg_n, ref logic [15:0] operands,
    ...
    unique case (opcode)
      ...
      8'h7D: begin
        if (fetched_data_bytes == 0) begin
          ...
          start_fetch_data(addr[14:0] & RAMW[14:0], adb, state, fetch_stage, fetch_resume_state);
        end else begin
          automatic logic [8:0] temp;
          temp = (ra + dout_r + (flg_c ? 1 : 0)) & 9'h1FF;
          flg_c = temp[8];
