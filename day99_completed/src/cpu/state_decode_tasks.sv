task automatic state_decode_execute();
  begin
    ...
    begin
      ...
      cpu_exec_transfers_pkg::exec_transfers(opcode, ra, rx, ry, sp, flg_z, flg_n, pc, pc_plus1,
                                             adb, next_state, next_fetch_stage, transfers_handled);
      ...
      if (!transfers_handled && !flags_custom_handled) begin
        cpu_exec_branches_pkg::exec_branches(opcode, flg_c, flg_z, flg_v, flg_n, operands, s_imm8,
                                             s_offset, addr, pc, pc_plus2, adb, next_state,
                                             next_fetch_stage, branches_handled);
      end

      if (!transfers_handled && !flags_custom_handled && !branches_handled) begin
        cpu_exec_compare_pkg::exec_compare(opcode, ra, rx, ry, flg_c, flg_z, flg_v, flg_n, operands,
                                           fetched_data_bytes, fetched_data, dout_r, pc, pc_plus2,
                                           pc_plus3, adb, next_state, next_fetch_stage,
      ...
      if (!transfers_handled && !flags_custom_handled && !branches_handled && !compare_handled) begin
        cpu_exec_logic_pkg::exec_logic(opcode, ra, rx, ry, flg_z, flg_n, operands,
                                       fetched_data_bytes, fetched_data, dout_r, pc, pc_plus2,
                                       pc_plus3, adb, next_state, next_fetch_stage,
                                       fetch_resume_state, logic_handled);
      end
      ...
      if (!transfers_handled && !flags_custom_handled && !branches_handled && !compare_handled && !logic_handled) begin
        cpu_exec_shifts_pkg::exec_shifts(opcode, ra, rx, flg_c, flg_z, flg_n, operands,
                                         ...
                                         pc_plus3, ada, din, adb, cea, v_cea, next_state,
      ...
      if (!transfers_handled && !flags_custom_handled && !branches_handled && !compare_handled && !logic_handled && !shifts_handled) begin
        cpu_exec_store_pkg::exec_store(
            opcode, ra, rx, ry, operands, fetched_data_bytes, fetched_data, dout_r, v_ada, v_din,
            v_cea, ada, din, cea, write_to_vram, pc, pc_plus1, pc_plus2, pc_plus3, adb, next_state,
            next_fetch_stage, fetch_resume_state, store_handled);
      end
      ...
      if (!transfers_handled && !flags_custom_handled && !branches_handled && !compare_handled && !logic_handled && !shifts_handled && !store_handled) begin
        cpu_exec_inc_dec_pkg::exec_inc_dec(opcode, rx, ry, flg_z, flg_n, operands,
                                           fetched_data_bytes, dout_r, adb, ada, din, cea, v_cea,
                                           pc, pc_plus1, pc_plus2, pc_plus3, next_state,
                                           next_fetch_stage, fetch_resume_state, inc_dec_handled);
      end
      ...
      if (!transfers_handled && !flags_custom_handled && !branches_handled && !compare_handled
        ...
        cpu_exec_control_flow_pkg::exec_control_flow(
            ...
            written_data_bytes, sp, ada, din, cea, v_cea, ra, flg_c, flg_z, flg_i, flg_d, flg_b,
            flg_v, flg_n, control_flow_handled);
      end
      ...
      if (!transfers_handled && !flags_custom_handled && !branches_handled && !compare_handled
        ...
        cpu_exec_load_store_pkg::exec_load_store(
            opcode, ra, rx, ry, flg_z, flg_n, operands, fetched_data_bytes, fetched_data, dout_r,
            pc, pc_plus1, pc_plus2, pc_plus3, adb, next_state, next_fetch_stage, fetch_resume_state,
            load_store_handled);
      end
      ...
      if (!transfers_handled && !flags_custom_handled && !branches_handled && !compare_handled
        ...
        cpu_exec_adc_sbc_pkg::exec_adc_sbc(opcode, ra, rx, ry, flg_c, flg_v, flg_z, flg_n, operands,
                                           fetched_data_bytes, fetched_data, dout_r, pc, pc_plus1,
                                           pc_plus2, pc_plus3, adb, next_state, next_fetch_stage,
                                           fetch_resume_state, adc_sbc_handled);
      end
