...
module tb_cpu_modules;
  ...
  task test_alu();
    ...
    if (result == 8'h80 && !carry_out && negative_flag && !zero_flag) begin
      ...
      $display("  FAIL: ADC operation incorrect, got 0x%02X, carry=%b, flags: Z=%b N=%b", result,
               carry_out, zero_flag, negative_flag);
      error_count++;
    end
    ...
    if (result == 8'h50 && carry_out && !zero_flag) begin  // A >= B, so carry set
      ...
      $display("  FAIL: CMP operation incorrect, result=0x%02X, C=%b, Z=%b", result, carry_out,
               zero_flag);
      error_count++;
    end
