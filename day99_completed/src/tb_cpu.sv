...
cpu dut (
    ...
    task check_cpu_state(input string test_name, input logic [7:0] expected_a,
                         input logic [7:0] expected_x, input logic [7:0] expected_y,
    ...
    initial begin
        ...
        $display(
            "  Load immediate test completed (detailed validation requires CPU signal access)");
        test_case++;
        ...
        if (v_ada != 10'h000 || v_din != 8'h00) begin
            $display("  PASS: VRAM operations detected - Address: 0x%03X, Data: 0x%02X", v_ada,
                     v_din);
        end else begin
    ...
    always @(posedge clk) begin
        ...
        if (v_cea) begin
            $display("VRAM Write: Address=0x%03X, Data=0x%02X (char='%c') at time %0t", v_ada,
                     v_din, (v_din >= 32 && v_din <= 126) ? v_din : 8'h2E, $time);
        end
    end
