...
**中級者**: [CPU 実装詳細](#cpu-実装詳細)と[メモリアーキテクチャ](#メモリアーキテクチャ)を学習
**上級者**: [カスタム命令](#カスタム命令)と[最適化手法](#性能最適化)を深く理解
...
- `state_decode` からの分岐は 3 列に整理され、各パッケージカテゴリの連続性を保ったまま横方向の広がりを抑えています。
...
3. **コピー処理 (FSM)**:
    - `src/cpu/cpu_fsm_next_pkg.sv` 内の FSM に `INIT_RAM` というステートが存在します。
    - リセット解除後、FSM は `INIT` から `INIT_RAM` へ遷移します。
    - このステートでは、`boot_program` 配列の内容を 1 バイトずつ読み出し、RAM の `0x0200` 番地（プログラム開始位置）以降へ書き込みます（`cea=1`, `din=boot_byte`）。

4. **実行開始**:
    - 全バイトのコピーが完了すると、FSM は `FETCH_REQ` ステートへ遷移し、プログラムカウンタ (PC) `0x0200` から通常の命令フェッチを開始します。

この方式により、専用のブートローダー回路（外部モジュール）を削減し、CPU の制御ロジック内で完結させています。
